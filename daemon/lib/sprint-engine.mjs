...
function syncItemToDb(item, projectRoot = '') {
  try {
    ...
    const userId = firstUser ? firstUser.id : 'system';
    prepare(`
      INSERT INTO sprint_items (
        id, user_id, project_root, type, title, description, priority, status,
        sprint_id, epic_id, initiative_id, auto_generated, related_files,
        evidence, dependencies, labels, storyPoints, assignee, createdAt, updatedAt
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
function deleteItemFromDb(id) {
  try {
    prepare('DELETE FROM sprint_items WHERE id = ?').run(id);
  } catch {}
}
...
export class SprintEngine {
  ...
  async createItem(attrs) {
    ...
    data.items[id] = item;
    await this._save();
    syncItemToDb(item, this.projectRoot);
    return item;
  }
  ...
  async estimateItem(itemId) {
    ...
    return {
      ...
      breakdown: {
        ...
        typeAdjustment: itemTypesAdjustment[item.type] || 1.0,
      },
    };
  }
  ...
  async autoPrioritize(opts = {}) {
    ...
    for (const item of items) {
      ...
      // Deadline scoring — items with nearer deadlines get higher priority
      if (opts.deadlines && item.deadline) {
        const deadline = new Date(item.deadline);
        const daysUntil = Math.max(0, (deadline.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
        const deadlineScore = Math.max(0, 30 - daysUntil * 2);
        score += deadlineScore;
        if (daysUntil <= 7) score += 20; // Urgency bonus for within-a-week deadlines
      }

      // Priority-based scoring (P0 = highest)
      const priorityScore = { P0: 100, P1: 80, P2: 60, P3: 30, P4: 15, P5: 5 };
      ...
      if (item.type === 'epic') score += 15;
      if (item.type === 'initiative') score += 25;
